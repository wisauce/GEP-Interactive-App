...
export default function Messaging({Messages, artworks, section}) {
  ...
  const routes = ["gate-of-ludic", "kreoskaura", "neoneshia", "mexirelia", "secret"]
  ...
  return visibleItems[groupIndex].includes(itemIndex) ? (
    <div key={`item-${groupIndex}-${itemIndex}`}>
      {item.type === "mascot" ? (
        <BubbleChatLeft
          text={item.text}
          section={section}
  ...
  ) : null; // This ensures that every .map iteration returns a valid value
})}
